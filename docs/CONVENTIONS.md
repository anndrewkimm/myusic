...
Hookline also has other ways to get audio in: importing a file already on disk (spec 008, widened in spec 017), importing directly from a URL (spec 018, decided 2026-07-30), and pasting a Spotify track link in that same URL-import dialog instead of a direct video URL (spec 021, decided 2026-07-31). That URL-fetch path genuinely does fetch content from a third-party service rather than recording local output, which is a different and higher-risk category than the live-capture path; spec 018 documents the specific caveats and constraints (single video at a time, no playlists, no bulk fetching, personal-use framing shown in-app).

Spec 021 never fetches audio from Spotify itself — Spotify's API only ever exposes metadata and, for some tracks, a 30-second preview, never full audio, and that's a licensing boundary, not a technical gap Hookline works around. What spec 021 actually does is read a Spotify link's public track metadata (title/artist, nothing account-specific), search YouTube for the matching video, and require the user to confirm the specific match before anything downloads — at which point it hands off to spec 018's existing fetch flow completely unchanged. It's a metadata-assisted convenience layer in front of spec 018, not a second fetch mechanism, and it inherits every one of spec 018's constraints and caveats unchanged, plus one further caveat unique to this path: the YouTube match is search-based and best-effort, never guaranteed to be the exact right recording, which is exactly why user confirmation before download is non-negotiable here.
